...
def make_position(fen):
    ...
    for i in range(8):
        ...
        for j in range(8):
            ...
            if (i+j) % 2 == 1:
                square = Image.open(os.path.join(chess_assets_path, 'square brown dark_1x.png'))
            
            else:
                square = Image.open(os.path.join(chess_assets_path, 'square brown light_1x.png'))

            if name != 'empty':
                piece = Image.open(os.path.join(chess_assets_path, f'{name}_1x.png'))
                x1 = int(.5 * square.size[0] - .5 * piece.size[0]) + 1
                y1 = int(.5 * square.size[1] - .5 * piece.size[1])
