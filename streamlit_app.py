...
from chessBoard import make_position
...
version_dict = {
    "basic-resnet18": ("1.0.0",),
    "augmented-resnet18":("1.0.0","1.1.0")
}
...
def load_model(workspace,model,version):
    ...
    try:
        if response['result'] == 'success':
            ...
            st.info(f'There was an error during the loading of the {model} model. Please try again or change model')
    except:
...
with st.sidebar:
    ...
    "Model",
    ("basic-resnet18", "augmented-resnet18")
    )
...
with st.container():
    ...
    if uploaded_file is not None and uploaded_file != prev_file:
        ...
        st.session_state.cache.update({'prev_file':uploaded_file, 'pred_fen': pred_fen, 'actual_fig': actual_fig, 'pred_fig':pred_fig})
...
with st.container():
    if pred_fen:
        ...
        with col1:
            st.markdown("<p style='text-align: center;'>Actual Position</p>", unsafe_allow_html=True)
            st.pyplot(actual_fig)

        with col2:
            st.markdown("<p style='text-align: center;'>Predicted Position</p>", unsafe_allow_html=True)
            st.pyplot(pred_fig)
